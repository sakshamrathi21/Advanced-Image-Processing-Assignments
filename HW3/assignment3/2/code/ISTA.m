...
barbaraOriginal = imread("barbara256.png");
% Convert the image to double for numerical precision and normalise it
barbaraOriginal = double(barbaraOriginal); 

%%

% Adding Gaussian Noise to the Image with mean 0, variance 4 on [0, 255]
noiseVariance = 4.0;
noise = noiseVariance * randn(size(barbaraOriginal));
barbaraNoisy = barbaraOriginal + noise;
...
% patches = mat2cell(barbaraNoisy, repmat(patchSize, 1, height / patchSize), repmat(patchSize, 1, width / patchSize));
...
% Check the sparsity of the patches in the DCT domain
...
% alpha = norm(A, 2)^2;
% Define lambda as 1.0
lambda = 1.0;
% Define the maximum iterations as 1000
maxIter = 1000;
disp("Type maxIter : " + class(maxIter));
...
for i = 1:numPatches
    ...
    reconstructedPatches(:, i) = DCT * patchISTA(measuredPatch, A, lambda, alpha, maxIter);
end
...
% reconstructedImage = reconstructFromPatchesNonOverlapping(distReconstructedPatches, [patchSize patchSize], [height width]);
...
noise = noiseVariance * randn(size(goldhillOriginal));
...
barbaraNoisy = barbaraOriginal + noiseVariance * randn(size(barbaraOriginal));
barbaraNoisy = barbaraNoisy / 256.0;
patches = im2col(barbaraNoisy, [patchSize, patchSize], 'sliding');
numPatches = size(patches, 2);
% Define the sensing matrix as a random normal 32*64 matrix
...
for i = 1:length(lambdaRange)
    ...
    for j = 1:numPatches
        currPatch = patches(:,j);
        measuredPatch = Phi * currPatch;
        reconstructedPatches(:, j) = DCT * patchISTA(measuredPatch, A, lambda, alpha, maxIter);
    end
    % Reconstruct the image from the patches
    reconstructedImage = reconstructFromPatches(reconstructedPatches, [patchSize patchSize], [height width]);
    % Compute the RMSE between the original image and the reconstructed image
...
grid on;
% Save the figure
saveas(gcf, "ISTA_RMSE_Barbara.png");
