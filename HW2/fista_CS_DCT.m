...
for f=1:lfrac
    ...
    for i=1:H-ps+1
        ...
        for j=1:W-ps+1
            ...
            [r,J] = fista(coded_patches(:,cnt),Phi*D,D'*Phi',lambda,alpha,200);
            rec_patches(:,cnt) = D*r;
    ...
    figure; imshow(im/max(im(:))); title ('original image');   
    figure;imshow(rec_im/max(rec_im(:))); title ('reconstructed image');
end
